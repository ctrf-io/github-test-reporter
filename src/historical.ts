...
export async function generateHistoricSummary(report: CtrfReport, artifactName: string, rows: number): Promise<void> {
    ...
    if (github?.runId && github.runNumber && github.buildUrl) {
        const extendedReport: CtrfReport & { runId: string, runNumber: string, buildUrl: string } = {
            ...report,
            runId: github.runId,
    ...
    const workflowRun = reports as Array<CtrfReport & { runId: string, runNumber: string, buildUrl: string }>;
...
async function fetchPreviousRuns(githubProperties: any) {
    ...
    const relevantRun = previousRuns.workflow_runs.find((run: any) => run.name === githubProperties.workflow);

    if (!relevantRun) {
        console.error(`No runs found for the workflow: ${githubProperties.workflow}`);
        return [];
    }
    ...
    const filteredRuns = await workflowRuns.workflow_runs.filter((run: any) => {
        const isBranchMatch = run.head_branch === githubProperties.branch && run.event === "push";
        const isPRMatch = run.event === 'pull_request' &&
            run.pull_requests.some((pr: any) => pr.number === githubProperties.pullRequestNumber);
...
async function fetchArtifactsFromPreviousBuilds(githubProperties: any, artifactName: string): Promise<(CtrfReport & { runId: string, runNumber: string, buildUrl: string })[]> {
    const previousRuns = await fetchPreviousRuns(githubProperties);

    const reports: (CtrfReport & { runId: string, runNumber: string, buildUrl: string })[] = [];
    ...
    for (const run of previousRuns) {
        ...
        for (const artifact of artifactsData.artifacts) {
            if (artifact.name === artifactName) {
                const artifactBuffer = await fetchArtifactRedirect(artifact.archive_download_url);

                const report = unzipArtifact(artifactBuffer);

                if (report) {
                    const extendedReport = report as CtrfReport & { runId: string, runNumber: string, buildUrl: string };
...
function fetchArtifactRedirect(url: string): Promise<Buffer> {
...
function downloadArtifact(url: string): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        https.get(url, (res) => {
            ...
            if (res.statusCode === 200) {
                ...
                res.on('end', () => {
                    ...
                    reject(new Error(`Failed to download artifact with status code ${res.statusCode}`));
                });
            }
        }).on('error', reject);
    });
